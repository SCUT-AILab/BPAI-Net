...
class I3D(torch.nn.Module):
    ...
    def forward(self, input, first=None, second=None):
        '''
        :param input: 输入
        :param first: 有三个值可选：conv3d_2c，mixed_3c，mixed_5c
        :param second: 有三个值可选：conv3d_2c，mixed_3c，mixed_5c
        :return:
        '''
        if args.arch == 'i3d_all':
            return self.forward_all(input,first,second)
        elif args.arch == 'i3d':
            return self.forward_rgb(input)

    def forward_all(self, inp,first=None,second=None):  # input torch.Size([8, 3, 64, 224, 224])
        # Preprocessing
        if first:
            out = self.conv3d_1a_7x7(inp)  # torch.Size([8, 64, 32, 112, 112])
            out = self.maxPool3d_2a_3x3(out)  # torch.Size([8, 64, 32, 56, 56])
            out = self.conv3d_2b_1x1(out)  # torch.Size([8, 64, 32, 56, 56])
            out = self.conv3d_2c_3x3(out)  # torch.Size([8, 192, 32, 56, 56])
            out = self.maxPool3d_3a_3x3(out)  # torch.Size([8, 192, 32, 28, 28])
            out = self.mixed_3b(out)  # torch.Size([8, 256, 32, 28, 28])
            out = self.mixed_3c(out)  # torch.Size([8, 480, 32, 28, 28])
            if first =='3c':
                rgb_first = out
            out = self.maxPool3d_4a_3x3(out)  # torch.Size([8, 480, 16, 14, 14])
            out = self.mixed_4b(out)  # torch.Size([8, 512, 16, 14, 14])
            out = self.mixed_4c(out)  # torch.Size([8, 512, 16, 14, 14])
            out = self.mixed_4d(out)  # torch.Size([8, 512, 16, 14, 14])
            out = self.mixed_4e(out)  # torch.Size([8, 528, 16, 14, 14])
            out = self.mixed_4f(out)  # torch.Size([8, 832, 16, 14, 14])
            out = self.maxPool3d_5a_2x2(out)  # torch.Size([8, 832, 8, 7, 7])
            out = self.mixed_5b(out)  # torch.Size([8, 832, 8, 7, 7])
            out = self.mixed_5c(out)  # torch.Size([8, 1024, 8, 7, 7])
            return rgb_first,out
        else:
            out = self.avg_pool(inp)
    ...
    def forward_rgb(self, inp):  # input torch.Size([8, 3, 64, 224, 224])
        # Preprocessing
